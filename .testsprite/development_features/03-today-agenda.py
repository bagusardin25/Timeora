...
async def run_test():
    ...
    try:
        ...
        await page.locator("#date").fill(date.today().isoformat())
        ...
        agenda_matches = page.get_by_text(TITLE, exact=True)
        await expect(agenda_matches.last).to_be_visible()
        assert await agenda_matches.count() >= 2, (
